...
duernast_dssat_fmt <- lapply(duernast_dssat, function(dataset) {
  compile_model_dataset(
    ...
    args = list(RSEED = 1234, FERTI = "Y", SDATE = 22151)
  )
})

# Example to edit table: add fertilizer depth
duernast_dssat$TUDU1601$MANAGEMENT$FERTILIZERS$FDEP <- 2

duernast_2016 <- compile_model_dataset(
  dataset = duernast_dssat$TUDU1601,
  framework = "dssat",
  sol_append = FALSE,
  write = TRUE, path = "./data/2_dssat",
  args = list(RSEED = 1234, FERTI = "Y", SDATE = 15300)
)
